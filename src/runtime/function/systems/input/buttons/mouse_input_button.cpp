...
namespace Meow
{
    MouseInputButton::MouseInputButton(MouseButtonCode button)
        : m_button(button)
    {
        g_runtime_global_context.render_context.window->OnMouseButton().connect(
    ...
    {
        return g_runtime_global_context.render_context.window->GetMouseButtonAction(m_button);
    }
} // namespace Meow
