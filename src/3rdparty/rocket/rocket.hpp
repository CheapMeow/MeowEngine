...
namespace rocket
{
    ...
    struct minimum
    {
        ...
        result_type result() { return std::move(current); }
    ...
    struct range
    {
        ...
        result_type result() { return std::move(values); }
    ...
    {
        const char* what() const ROCKET_NOEXCEPT override { return "rocket: Bad optional access."; }
    };
    ...
    struct optional final
    {
        ...
        bool engaged() const ROCKET_NOEXCEPT { return initialized; }

        bool has_value() const ROCKET_NOEXCEPT { return initialized; }

        explicit operator bool() const ROCKET_NOEXCEPT { return engaged(); }

        value_type& operator*() ROCKET_NOEXCEPT { return value(); }

        value_type const& operator*() const ROCKET_NOEXCEPT { return value(); }

        value_type* operator->() ROCKET_NOEXCEPT { return object(); }

        value_type const* operator->() const ROCKET_NOEXCEPT { return object(); }
...
#    endif
    ...
    private:
        void* storage() ROCKET_NOEXCEPT { return static_cast<void*>(&buffer); }

        void const* storage() const ROCKET_NOEXCEPT { return static_cast<void const*>(&buffer); }
    ...
    struct intrusive_ptr final
    {
        ...
        constexpr intrusive_ptr() ROCKET_NOEXCEPT : ptr {nullptr} {}

        constexpr intrusive_ptr(std::nullptr_t) ROCKET_NOEXCEPT : ptr {nullptr} {}

        explicit intrusive_ptr(pointer p) ROCKET_NOEXCEPT : ptr {p}
        {
            if (ptr)
            {
                ptr->addref();
        ...
        intrusive_ptr(intrusive_ptr const& p) ROCKET_NOEXCEPT : ptr {p.ptr}
        {
            if (ptr)
            {
                ptr->addref();
        ...
        intrusive_ptr(intrusive_ptr&& p) ROCKET_NOEXCEPT : ptr {p.ptr} { p.ptr = nullptr; }

        template<class U>
        explicit intrusive_ptr(intrusive_ptr<U> const& p) ROCKET_NOEXCEPT : ptr {p.ptr}
        {
            if (ptr)
            {
                ptr->addref();
        ...
        template<class U>
        explicit intrusive_ptr(intrusive_ptr<U>&& p) ROCKET_NOEXCEPT : ptr {p.ptr}
        {
            p.ptr = nullptr;
        }
        ...
        pointer get() const ROCKET_NOEXCEPT { return ptr; }

        pointer get() const volatile ROCKET_NOEXCEPT { return ptr; }
        ...
        operator pointer() const ROCKET_NOEXCEPT { return ptr; }

        operator pointer() const volatile ROCKET_NOEXCEPT { return ptr; }
        ...
        pointer const* operator&() const ROCKET_NOEXCEPT { return &ptr; }
        ...
        intrusive_ptr& operator=(intrusive_ptr const& p) ROCKET_NOEXCEPT { return (*this = p.ptr); }
        ...
        void swap(intrusive_ptr& p) ROCKET_NOEXCEPT { swap(&p.ptr); }
    ...
    {
        unsigned long addref() ROCKET_NOEXCEPT { return ++count; }

        unsigned long release() ROCKET_NOEXCEPT { return --count; }

        unsigned long get() const ROCKET_NOEXCEPT { return count; }
    ...
    struct ref_count_atomic final
    {
        ...
        unsigned long get() const ROCKET_NOEXCEPT { return count.load(std::memory_order_relaxed); }
    ...
    struct ref_counted
    {
        ...
        ref_counted& operator=(ref_counted const&) ROCKET_NOEXCEPT { return *this; }

        void addref() ROCKET_NOEXCEPT { count.addref(); }
    ...
    class stable_list final
    {
        struct link_element final : ref_counted<link_element>
        {
            ...
            T* value() ROCKET_NOEXCEPT { return static_cast<T*>(storage()); }
    ...
    public:
        ...
        struct iterator_base final
        {
            ...
            iterator_base(iterator_base const& i) ROCKET_NOEXCEPT : element {i.element} {}

            iterator_base(iterator_base&& i) ROCKET_NOEXCEPT : element {std::move(i.element)} {}

            template<class V>
            explicit iterator_base(iterator_base<V> const& i) ROCKET_NOEXCEPT : element {i.element}
            {}

            template<class V>
            explicit iterator_base(iterator_base<V>&& i) ROCKET_NOEXCEPT : element {std::move(i.element)}
            {}
            ...
            reference operator*() const ROCKET_NOEXCEPT { return *element->value(); }

            pointer operator->() const ROCKET_NOEXCEPT { return element->value(); }
        ...
        private:
            ...
            iterator_base(link_element* p) ROCKET_NOEXCEPT : element {p} {}
        };
        ...
        iterator begin() ROCKET_NOEXCEPT { return iterator {head->next}; }

        iterator end() ROCKET_NOEXCEPT { return iterator {tail}; }

        const_iterator begin() const ROCKET_NOEXCEPT { return const_iterator {head->next}; }

        const_iterator end() const ROCKET_NOEXCEPT { return const_iterator {tail}; }

        const_iterator cbegin() const ROCKET_NOEXCEPT { return const_iterator {head->next}; }

        const_iterator cend() const ROCKET_NOEXCEPT { return const_iterator {tail}; }

        reverse_iterator rbegin() ROCKET_NOEXCEPT { return reverse_iterator {end()}; }

        reverse_iterator rend() ROCKET_NOEXCEPT { return reverse_iterator {begin()}; }

        const_reverse_iterator rbegin() const ROCKET_NOEXCEPT { return const_reverse_iterator {cend()}; }

        const_reverse_iterator rend() const ROCKET_NOEXCEPT { return const_reverse_iterator {cbegin()}; }

        const_reverse_iterator crbegin() const ROCKET_NOEXCEPT { return const_reverse_iterator {cend()}; }

        const_reverse_iterator crend() const ROCKET_NOEXCEPT { return const_reverse_iterator {cbegin()}; }

        reference front() ROCKET_NOEXCEPT { return *begin(); }

        reference back() ROCKET_NOEXCEPT { return *rbegin(); }

        value_type const& front() const ROCKET_NOEXCEPT { return *cbegin(); }

        value_type const& back() const ROCKET_NOEXCEPT { return *crbegin(); }

        bool empty() const ROCKET_NOEXCEPT { return cbegin() == cend(); }

        void clear() ROCKET_NOEXCEPT { erase(begin(), end()); }
        ...
        size_type size() const ROCKET_NOEXCEPT { return elements; }

        size_type max_size() const ROCKET_NOEXCEPT { return std::numeric_limits<size_type>::max(); }
    ...
    namespace detail
    {
        ...
        struct shared_lock_state<thread_unsafe_policy> final
        {
            ...
            constexpr bool try_lock() ROCKET_NOEXCEPT { return true; }
        ...
        struct shared_lock_state<thread_safe_policy> final
        {
            ...
            void swap(shared_lock_state& s) ROCKET_NOEXCEPT { lock_primitive.swap(s.lock_primitive); }
        ...
        template<>
        struct connection_base<thread_unsafe_policy> : thread_unsafe_policy,
                                                       ref_counted<connection_base<thread_unsafe_policy>>
        {
            virtual ~connection_base() ROCKET_NOEXCEPT = default;

            bool is_connected() const ROCKET_NOEXCEPT { return prev != nullptr; }
            ...
            std::thread::id get_tid() const ROCKET_NOEXCEPT { return std::thread::id {}; }

            constexpr bool is_queued() const ROCKET_NOEXCEPT { return false; }

#ifndef ROCKET_NO_BLOCKING_CONNECTIONS
            void block() ROCKET_NOEXCEPT { ++block_count; }
            ...
            bool is_blocked() const ROCKET_NOEXCEPT { return block_count > 0; }
        ...
        template<>
        struct connection_base<thread_safe_policy> : thread_safe_policy,
                                                     ref_counted<connection_base<thread_safe_policy>, ref_count_atomic>
        {
            ...
            std::thread::id const& get_tid() const ROCKET_NOEXCEPT { return thread_id; }
        ...
        {
            connection_scope(void* base, thread_local_data* th) ROCKET_NOEXCEPT : th {th}, prev {th->current_connection}
            {
                th->current_connection = base;
            }

            ~connection_scope() ROCKET_NOEXCEPT { th->current_connection = prev; }
        ...
        {
            abort_scope(thread_local_data* th) ROCKET_NOEXCEPT : th {th}, prev {th->emission_aborted}
            {
                th->emission_aborted = false;
            }

            ~abort_scope() ROCKET_NOEXCEPT { th->emission_aborted = prev; }
    ...
    struct connection
    {
        connection() ROCKET_NOEXCEPT : base {nullptr} {}

        ~connection() ROCKET_NOEXCEPT { release(); }

        connection(connection&& rhs) ROCKET_NOEXCEPT : base {rhs.base} { rhs.base = nullptr; }

        connection(connection const& rhs) ROCKET_NOEXCEPT : base {rhs.base} { addref(); }

        explicit connection(void* base) ROCKET_NOEXCEPT : base {base} { addref(); }
        ...
        bool operator==(connection const& rhs) const ROCKET_NOEXCEPT { return base == rhs.base; }

        bool operator!=(connection const& rhs) const ROCKET_NOEXCEPT { return base != rhs.base; }

        explicit operator bool() const ROCKET_NOEXCEPT { return is_connected(); }
        ...
        bool is_connected() const ROCKET_NOEXCEPT
        {
            if (base != nullptr)
            {
                ...
                if (!unsafe->is_thread_safe)
                    ROCKET_LIKELY { return unsafe->is_connected(); }
                else
                {
        ...
        bool is_blocked() const ROCKET_NOEXCEPT
        {
            if (base != nullptr)
            {
                ...
                if (!unsafe->is_thread_safe)
                    ROCKET_LIKELY { return unsafe->is_blocked(); }
                else
                {
        ...
        void disconnect() ROCKET_NOEXCEPT
        {
            if (base != nullptr)
            {
                ...
                if (!unsafe->is_thread_safe)
                    ROCKET_LIKELY { unsafe->disconnect(); }
                else
                {
                    safe->disconnect();
                }
    ...
    struct scoped_connection final : connection
    {
        ...
        ~scoped_connection() ROCKET_NOEXCEPT { disconnect(); }

        scoped_connection(connection const& rhs) ROCKET_NOEXCEPT : connection {rhs} {}

        scoped_connection(connection&& rhs) ROCKET_NOEXCEPT : connection {std::move(rhs)} {}

        scoped_connection(scoped_connection&& rhs) ROCKET_NOEXCEPT : connection {std::move(rhs)} {}
    ...
    struct scoped_connection_container final
    {
        ...
        void disconnect() ROCKET_NOEXCEPT { connections.clear(); }
    ...
    struct trackable
    {
        ...
        void disconnect_tracked_connections() ROCKET_NOEXCEPT { container.disconnect(); }
    ...
    inline void abort_emission() ROCKET_NOEXCEPT { detail::get_thread_local_data()->emission_aborted = true; }

#ifndef ROCKET_NO_BLOCKING_CONNECTIONS
    struct scoped_connection_blocker final
    {
        scoped_connection_blocker(connection c) ROCKET_NOEXCEPT : conn {std::move(c)} { conn.block(); }

        ~scoped_connection_blocker() ROCKET_NOEXCEPT { conn.unblock(); }
    ...
    namespace detail
    {
        ...
        struct timer_queue final
        {
            ...
            ~timer_queue() ROCKET_NOEXCEPT { destroy(); }
            ...
            connection
            set_interval(Instance& object, R (Class::*method)(), std::chrono::duration<Rep, Period> const& interval)
            {
            ...
            template<class Instance, class Class, class R, class Rep = unsigned long, class Period = std::milli>
            connection
            set_timeout(Instance& object, R (Class::*method)(), std::chrono::duration<Rep, Period> const& timeout)
            {
...
#    endif
                            ...
                            ROCKET_LIKELY
                            {
                                ...
                                timed_connection* conn = static_cast<timed_connection*>(static_cast<void*>(current));
...
#    endif
        ...
        private:
            ...
            timed_connection* make_link(connection_base*                                   l,
                                        slot_type                                          slot,
                                        std::chrono::time_point<std::chrono::steady_clock> expires_at,
                                        std::chrono::microseconds                          interval)
            {
    ...
    {
        return detail::get_timer_queue()->template set_interval<>(std::move(slot),
    ...
    {
        return detail::get_timer_queue()->template set_timeout<>(std::move(slot),
    ...
    inline void clear_timers() ROCKET_NOEXCEPT { detail::get_timer_queue()->clear(); }
#endif //~ ROCKET_NO_TIMERS
    ...
    struct signal<R(Args...), Collector, ThreadingPolicy> final
    {
        ...
        signal(signal&& s)
        {
            static_assert(std::is_same_v<ThreadingPolicy, thread_unsafe_policy>,
                    ...
                    if (current->prev != nullptr
                        ...
                        ROCKET_LIKELY
                        {
                            ...
                            functional_connection* conn =
                                static_cast<functional_connection*>(static_cast<void*>(current));
                            ...
                            else
                            {
                                if (current->is_queued())
                                    ROCKET_UNLIKELY
                                    {
                                        if constexpr (std::is_void_v<R>)
                                        {
                                            std::packaged_task<void()> task([current,
                                                                             args = detail::make_tuple(args...)] {
                                                if (current->is_connected())
                                                    ROCKET_LIKELY
                                                    {
                                                        ...
                                                        functional_connection* conn =
                                                            static_cast<functional_connection*>(
    ...
    private:
        ...
        void copy(signal const& s)
        {
            ...
            while (current != end)
            {
                functional_connection* conn = static_cast<functional_connection*>(static_cast<void*>(current));
    ...
    inline void swap(connection& c1, connection& c2) ROCKET_NOEXCEPT { c1.swap(c2); }
    ...
    inline void swap(signal<Signature, Collector, ThreadingPolicy>& s1,
                     signal<Signature, Collector, ThreadingPolicy>& s2) ROCKET_NOEXCEPT
    {
        s1.swap(s2);
    }
} // namespace rocket
